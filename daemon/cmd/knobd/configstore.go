...
// sync. It is also the single entry point PUT /config and a SIGHUP
// reload both go through.
...
func newConfigStore(path string, cfg model.Config, eng configApplier, log *slog.Logger) *configStore {
	return &configStore{path: path, cfg: cfg, eng: eng, log: log}
}
...
func (s *configStore) SetConfig(ctx context.Context, cfg model.Config) error {
	...
	if err := s.eng.SetConfig(ctx, cfg); err != nil {
		return fmt.Errorf("configstore: apply to engine: %w", err)
	}
